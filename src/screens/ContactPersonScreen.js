...
const setContactPerson= (value) => dispatch({ type: 'SET_CONTACT_PERSON', payload: { ...value } })
    ...
    const mykadError = error && errorColor.find(test => test == "MyKad")
    if (mykadError == "MyKad") {
        mykadBorderColor = '#d94498'
    }

    var positionBorderColor = '#5a83c2'
    const positionError = error && errorColor.find(test => test == "Position")
                ...
                <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
                    <View style={{ width: Layout.window.width * 0.8, justifyContent: 'center', alignItems: 'center' }}>
                        ...
                        <View style={{ alignSelf: 'center', borderBottomWidth: 1, borderBottomColor: '#4A90E2', flexDirection: 'row', margin: 5, width: Layout.window.width * 0.65 }}>
                            ...
                            <TextInput value={position} onChangeText={(position) => setContactPerson({ position })} value={contactPosition} style={{ marginLeft: 5, flex: 1 }} placeholder={(positionErrorHint.length > 0) ? positionErrorHint : 'Position'} placeholderTextColor={(positionErrorHint.length > 0) ? 'rgba(255,0,0,0.3)' : 'lightgrey'} />
                        </View>
